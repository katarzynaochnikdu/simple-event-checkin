...
**Status:** ✅ DONE 2026-06-10 (gate fali mobile 033+034 PASS 0/0/0/2; committed mobile wave) — 8 punktów: N-3 SecureDialogEffect (core-ui, FLAG_SECURE na AddOrderSheet/ChangePasswordDialog/ScannerScreen dialogi/WalkinFormSheet, gated !DEBUG) · F2A-007 ProGuard zawężony (drop Gson/Room keepy, `keep core.network.dto.**` zamiast `.network.**`, drop `core.model.**`; Moshi codegen reflection-free zweryfikowany) · F2A-008 data_extraction_rules.xml + camera uses-feature · F2A-011 openExternalUrl allowlist http(s) (SpeakerDetail social/website + ConsentsForm) · F2B-006 QR charset regex `^[A-Za-z0-9_-]{1,100}$` · F2B-005 log hygiene (DEBUG-guard + printStackTrace removal) · F2A-012 dead-module guard-komentarze (walkin/inhub/sponsors + ImagePickerCropper; wariant b) · F2A-010 Uri.encode(token) + RESET_TOKEN_REGEX walidacja w AppNavHost. **assembleDebug PASS 2m7s + assembleRelease (R8) PASS 8m35s** (zawężone keep-rules nie łamią deserializacji). 21 plików +233/-46 + 3 nowe. Pliki WO-MOB-033/030/031 nietknięte. Gotcha-kandydaci: ProGuard/Moshi codegen vs reflective fallback; Compose Dialog nie dziedziczy Activity FLAG_SECURE. Smoke release on-device (login→lista→skan) = post-deploy user (worker nie ma urządzenia).
